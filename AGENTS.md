...
43. **An API response must never fail because of a field**: ZoneMinder changes what it sends between releases. Fields we do not declare are already safe (Zod strips unknown keys; keep it that way, never `.strict()`). Every object schema in `api/` wraps its fields with `withFieldCatch(shape, identity)` and every response array uses `tolerantArray(itemSchema, label)` (both in `lib/zm/schema-tolerance.ts`): a drifted field falls back to a type-matching default, and a row that still fails on its identity (`Id`/`Name`) drops itself rather than blanking the list. A value ZoneMinder controls the vocabulary of (a mode, a type, a status) is `z.string()`, never `z.enum()`; keep known values as a plain array for pickers. Be tolerant reading, strict writing: narrow unions still belong on values we SEND. Why: ZM 1.38.3 changed `V4LMultiBuffer` to boolean `false`, and a field the app never reads blanked every camera (#247); an enum defaulted on an unknown value would be worse than failing (it would report a recording camera as off). `api/__tests__/types.test.ts` enforces this per field across every schema by reading field names off `.shape`; add a case there when adding a schema.
