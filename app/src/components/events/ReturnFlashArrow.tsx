...
export function ReturnFlashArrow({ className }: { className?: string }) {
  return (
    <ChevronRight
      ...
      className={cn(
        'pointer-events-none absolute left-1 top-1/2 z-20 -translate-y-1/2 h-6 w-6 rounded-full bg-background/70 p-0.5 text-primary shadow drop-shadow motion-safe:animate-blink',
        className
      )}
    />
  );
