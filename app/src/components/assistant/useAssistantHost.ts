 ...
 * (`stores/assistantPanel.ts`) before routing, so an "Open" result-card click
 * (`AssistantResultCards`) collapses it to the FAB instead of closing it: the
 * conversation underneath survives the navigation.
 *
...
export function useAssistantHost() {
  ...
  const setPhase = useAssistantStore((s) => s.setPhase);
