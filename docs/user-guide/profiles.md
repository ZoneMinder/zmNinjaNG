...
Preferences are two-tier: All Servers mode has its own settings (grid layout, feed fit, and so on), kept separate from each individual profile's settings. A change made while in All Servers mode does not touch any single profile's settings, and vice versa. This covers what you are looking at rather than how a server is reached, so while aggregating, one *Streaming Mode* and one analysis-frames setting govern every tile no matter which server it came from, rather than each tile following its own server's choice. Timeouts, ports and other connection settings still belong to each server.
