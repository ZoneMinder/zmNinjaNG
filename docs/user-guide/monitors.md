...
The maximize button on the Monitor Detail toolbar fills the screen with the feed and hides everything else. The app remembers that per monitor: a monitor you left maximized opens maximized next time, including when you tap it from the Montage, until you exit with the button. On a phone or tablet, turning the device to landscape also fills the screen for as long as it stays landscape; that is temporary and does not change what the app remembers. Leaving fullscreen while landscape keeps the normal view until the next rotation.
