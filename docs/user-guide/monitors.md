...
The small info button next to each monitor's name, on the Monitors page and at the top of Monitor Detail, opens the monitor's capture settings with their names: Capturing, Analysing, Recording and Decoding on ZoneMinder 1.38 and later, or Function on older servers, plus the resolution and frame-rate cap. Hover it with a mouse or tap it on a phone. When Decoding is anything other than *Always*, the popover says so in one line: the monitor decodes only while someone is watching, which is why live snapshots on such a monitor pause between viewers (see the Streaming Mode note in {doc}`settings`). The same Decoding value is listed read-only on the Capture tab of the monitor's Settings dialog.
