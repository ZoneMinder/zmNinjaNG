...
The toggle is remembered per profile: once turned on it stays on for future events. The playback speed you pick is also remembered and reused for each event in a run, on both the MP4 and ZMS players. So is the MP4 player's mute state: unmute once and every event you open afterwards on that profile starts with audio on, until you mute again. Entering fullscreen on the player is remembered too: later events open fullscreen until you turn off **Open events in fullscreen** under Playback in {doc}`settings`. Leaving fullscreen changes only the event you are watching, since leaving is the only way off the page. Turning a phone to landscape also plays the event fullscreen for as long as it stays landscape. Some platforms refuse fullscreen the app enters on its own, and there the player fills the window instead. ZMS playback goes fullscreen the same way, with the picture filling the screen, the transport controls below it, and an Exit button at the top.
